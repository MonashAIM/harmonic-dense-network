...
There is a pipeline to check for test coverage and ruff lint, if the pipeline fails, please make sure to remedy it with a new push
  ...
  ##  DVC Setup
  ### Dependencies
  We will need to first install DVC first based on your OS. The guide for Windows is here:
  https://dvc.org/doc/install/windows
  You don't really need to install mamba but it does fasten up some downloading process. Nevertheless, the most important step is:
``` console
conda/mamba install -c conda-forge dvc
```
Then, make sure you have the extension for DVC in VSCode as well: https://marketplace.visualstudio.com/items?itemName=Iterative.dvc

  ### Establish connection and authorization
  Please contact Triet when you reach this step to retrieve a config file and provide a an email address to access the storage. Put this file into .dvc folder. Then, run the command:
 ``` console
dvc pull
```
Then the terminal will open a connection for you to approve access with your email address.

   ### Add a new model to be tracked
