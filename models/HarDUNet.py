...
class HarDUNet(nn.Module):
    def __init__(self, n_classes=1, arch='68', act='relu', transformer=False, keepbase=False, bilinear=True, *args, **kwargs):
        ...
        for j in range(blocks-2, -1, -1):
            ...
            # print(ch)
            block = Up(ch, n_layers[j-1], gr[j-1], m, act=act, dwconv=depthwise, keepbase=keepbase, bilinear=True)
            ch = block.get_out_ch()
        ...
        block = Up(first_ch[1], n_layers[0], gr[0], m, act=act, dwconv=depthwise, keepbase=keepbase, bilinear=True)
        ch = block.get_out_ch()
        self.dec.append(block)
        self.dec.append(Conv(ch, first_ch[1], act=act, kernel=1))
        ...
        if bilinear:
            self.bottleneck.append(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True))
        else:
            self.bottleneck.append(nn.ConvTranspose2d(ch_list[-1], ch_list[-1] // 2, kernel_size=2, stride=2))
